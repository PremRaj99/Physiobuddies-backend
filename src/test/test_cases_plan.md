...
- **Expected Outcome**:
  ...
  2. Patient B receives HTTP 409 Conflict / `ValidationError` with message _"Slot is currently on hold by another user"_.
...
- **Expected Outcome**: Returns HTTP 400 / 404 `ValidationError`: _"Reservation session has expired. Please re-select a slot."_
...
- **Expected Outcome**: HTTP 400 `ValidationError`: _"OTP can only be generated within 15 minutes of session start time."_
...
- **Expected Outcome**:
  1. HTTP 400 `ValidationError`: _"Invalid OTP. 2 attempts remaining."_
...
- **Expected Outcome**: HTTP 409 Conflict / `ValidationError`: _"Target slot is not available."_
...
- **Expected Outcome**:
  1. System blocks action with HTTP 400 `ValidationError`: _"Sessions cannot be cancelled by therapist within 1 hour of start time. Contact admin."_
...
- **Expected Outcome**: Returns HTTP 400 `ValidationError`: _"Clinical Assessment Report is mandatory for the 1st session before completion."_
...
- **Expected Outcome**: HTTP 409 Conflict / `ValidationError`: _"Review already submitted for this session."_
...
- **Expected Outcome**:
  ...
  2. Reason logged as _"Auto-completed due to 7 days of inactivity post 1st session"_.
...
| `TC-EDGE-001`  | Database       | Invalid 24-character hex ObjectId string passed in route parameter | HTTP 400 `ValidationError`: _"Invalid ObjectId format"_                      |
