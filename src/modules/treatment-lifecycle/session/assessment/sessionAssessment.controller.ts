...
class SessionAssessmentController {
    getAssessment = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
        const sessionId = req.params.id;
        const assessment = await sessionAssessmentService.getAssessment(sessionId);
        res.json(new OkResponse(assessment));
    });

    createOrUpdateAssessment = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
        const sessionId = req.params.id;
