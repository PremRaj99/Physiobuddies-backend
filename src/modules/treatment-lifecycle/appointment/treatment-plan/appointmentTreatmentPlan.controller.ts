...
class AppointmentTreatmentPlanController {
    createOrUpdateTreatmentPlan = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
        const appointmentId = req.params.id;
        const treatmentPlanData = req.body;
        await appointmentTreatmentPlanService.createOrUpdateTreatmentPlan(appointmentId, treatmentPlanData);
        res.json(new AcceptedResponse());
    });

    addSessionToTreatmentPlan = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
        const appointmentId = req.params.id;
