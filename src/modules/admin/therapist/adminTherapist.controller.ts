...
import { UpdateAdminTherapistSchema, UpdateCommissionRateSchema } from './adminTherapist.types';
...
class AdminTherapistController {
  ...
  updateTherapist = asyncHandler(async (req, res, _next) => {
    const therapistId = validateSchema(ObjectIdSchema, req.params.id);
    const parseData = validateSchema(UpdateAdminTherapistSchema, req.body);
    await adminTherapistService.updateTherapist(therapistId, parseData);
    return new AcceptedResponse('Therapist verified successfully').send(res);
  });
