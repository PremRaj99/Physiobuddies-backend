...
class TherapistEarningController {
    getEarnings = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    getEarningsSummary = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
        const therapistId = req.user.id; // Assuming therapist ID is available in the authenticated user object
        const summary = await therapistEarningService.getEarningsSummary(therapistId);
        res.json(new OkResponse(summary));
    });

    getEarningsBySession = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
        const therapistId = req.user.id; // Assuming therapist ID is available in the authenticated user object
